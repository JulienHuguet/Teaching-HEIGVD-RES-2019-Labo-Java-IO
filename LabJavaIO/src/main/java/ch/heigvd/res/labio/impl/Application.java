...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    for(String quoteTag : quote.getTags()){
      path += File.separator + quoteTag;
    }

    File quoteDirectory = new File(path);
    quoteDirectory.mkdirs();
      ...
      public void visit(File file) {
         ...
         */
          try{
            ...
            LOG.log(Level.SEVERE, "Error path can't find the file : {0}", ex.getMessage());
          }
      }
    });
  }
